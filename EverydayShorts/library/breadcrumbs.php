...
function theme_breadcrumbs_items($args) {
    ...
    if (!is_front_page()) {
        $items[] = theme_breadcrumbs_link($args, get_home_url(), '', __('Home', 'everydayshorts'));
    }
    ...
    if (is_tag()) {
        $items[] = theme_breadcrumbs_text($args, __("Tag: ", 'everydayshorts') . single_tag_title('', FALSE));
    }
    if (is_404()) {
        $items[] = theme_breadcrumbs_text($args, __("404 - Page not Found", 'everydayshorts'));
    }
    if (is_search()) {
        $items[] = theme_breadcrumbs_text($args, __("Search", 'everydayshorts'));
    }
    if (is_year()) {
        $items[] = theme_breadcrumbs_text($args, get_the_time('Y'));
    }
    if (is_author()) {
        $items[] = theme_breadcrumbs_text($args, sprintf(esc_attr(__('View all posts by %s', 'everydayshorts')), get_the_author()));
    }
